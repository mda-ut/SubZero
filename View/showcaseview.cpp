...
void ShowCaseView::initialize_VC_Connection(controller *controller)
{
    ...
    //Links the 1st 3 buttons of view to controller's first Hello, Numb, Bye Tasks

    connect(leftButton, SIGNAL(clicked()), controller, controller::addTaskToQueue(new Task::TaskCCR));
    connect(rightButton, SIGNAL(clicked()), controller, controller::addTaskToQueue(new Task::TaskHelloWorld));
    connect(surfaceButton, SIGNAL(clicked()), controller, controller::addTaskToQueue(new Task::TaskHeyTonight));
